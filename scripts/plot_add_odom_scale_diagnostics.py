...
def _parse_kv(message: str) -> Dict[str, str]:
    ...
    # This is sufficient for ADD_ODOM_SCALE and scalar ADD_ODOM_TWIST fields.
    kv = {}
...
def _load_event_metrics(event_path: str):
    ...
    with open(event_path, "r", encoding="utf-8") as f:
        for line_no, raw in enumerate(f, start=1):
            ...
            elif "[ADD_ODOM_TWIST]" in message:
                twist_rows.append(
                    {
                        "stamp": stamp,
    ...
    if not scale_rows:
        raise ValueError(
            "No [ADD_ODOM_SCALE] events found. Ensure diagnostics_write_event=true and run includes degeneracy frames."
        )
...
def _print_summary(scale_rows, twist_rows) -> None:
    ...
    if twist_rows:
        ...
        if lin_vals:
            print(f"- mean add-odom lin_norm (from [ADD_ODOM_TWIST]): {sum(lin_vals) / len(lin_vals):.4f}")
        if ang_vals:
            print(f"- mean add-odom ang_norm (from [ADD_ODOM_TWIST]): {sum(ang_vals) / len(ang_vals):.4f}")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Visualize add-odometry scale diagnostics from LIORF event.txt"
    )
    parser.add_argument(
        "--input",
        default="",
    ...
    parser.add_argument(
        "--output",
        default="",
        help="Output image path (default: <run_dir>/add_odom_scale_diagnostics.png)",
    )
    parser.add_argument(
        "--show",
        action="store_true",
    ...
    try:
        ...
        lidar_nondeg_speed = [_nan_safe_div(m, dt) for m, dt in zip(lidar_nondeg_m, dt_scan_s)]
        add_nondeg_speed = [_nan_safe_div(m, dt) for m, dt in zip(add_nondeg_m, dt_scan_s)]
        ...
        # 4) Raw add-odom twist telemetry
        ax = axes[3]
        if t_twist:
            ax.plot(t_twist, lin_norm, label="ADD_ODOM_TWIST lin_norm", color="tab:green")
            ax.plot(t_twist, ang_norm, label="ADD_ODOM_TWIST ang_norm", color="tab:brown")
            ax.legend(loc="best")
        else:
            ax.text(0.01, 0.5, "No [ADD_ODOM_TWIST] events found", transform=ax.transAxes, va="center")
        ...
        output_path = args.output if args.output else os.path.join(run_dir, "add_odom_scale_diagnostics.png")
